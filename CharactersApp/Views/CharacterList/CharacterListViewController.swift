...
extension CharacterListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .iPad:
            if let existingNavController = splitViewController?.viewControllers.last as? UINavigationController {
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let detailViewController = storyboard.instantiateViewController(withIdentifier: "CharacterDetailViewController") as! CharacterDetailViewController
                detailViewController.character = selectedCharacter
                let newNavController = UINavigationController(rootViewController: detailViewController)
                splitViewController?.preferredDisplayMode = .automatic
                splitViewController?.showDetailViewController(newNavController, sender: nil)
            }
        }
    }
}
